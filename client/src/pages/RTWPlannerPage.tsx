...
export default function RTWPlannerPage() {
  ...
  const calculateRecoveryProgress = (dateOfInjury: string, workStatus?: string, rtwPlanStatus?: string) => {
    // If worker is still off work, their actual recovery progress is 0%
    // regardless of how much time has elapsed since injury.
    if (workStatus === "Off work" && rtwPlanStatus !== "completed") return 0;

    // If RTW is completed, progress is 100%
    if (rtwPlanStatus === "completed") return 100;

    // For at-work cases, show time-based progress toward expected recovery
    ...
    return Math.min(95, Math.round((weeksElapsed / expectedWeeks) * 100));
  };
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {calculateRecoveryProgress(workerCase.dateOfInjury, workerCase.workStatus, workerCase.rtwPlanStatus)}%
                      </span>
                    </div>
                    <Progress value={calculateRecoveryProgress(workerCase.dateOfInjury, workerCase.workStatus, workerCase.rtwPlanStatus)} />
                  </div>
