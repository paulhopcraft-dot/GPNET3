...
if [ -z "$PLAN_FILE" ]; then
  ...
  if [ -n "$FEATURE" ]; then
    ...
    # Enhanced detection: Find any feature-specific plans
    FEATURE_PLANS=($(find .ai/ -name "*-plan.md" 2>/dev/null | head -5))

    if [ ${#FEATURE_PLANS[@]} -eq 1 ]; then
      # Single feature plan found
      PLAN_FILE="${FEATURE_PLANS[0]}"
      echo "🎯 Found feature plan: $(basename "$PLAN_FILE")"
    elif [ ${#FEATURE_PLANS[@]} -gt 1 ]; then
      # Multiple feature plans - let user choose
      echo "⚠️  Multiple plan files found:"
      for i in "${!FEATURE_PLANS[@]}"; do
        echo "   $((i+1)). $(basename "${FEATURE_PLANS[i]}")"
      done
      echo ""
      echo "Please specify which plan to use with --plan=filename"
      exit 1
      ...
      echo "   .ai/plan.md"
      echo "   .ai/[feature]-plan.md"
      echo ""
      echo "Create a plan first by asking Claude: 'Use the plan-writer skill to create a plan for: [description]'"
      exit 1
    fi
  fi
fi
