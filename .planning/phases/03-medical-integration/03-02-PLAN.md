...
1. Import combineRestrictions from "./services/restrictionMapper"
2. Import FunctionalRestrictions type from @shared/schema
...
5. Aggregation logic:
- If single certificate with functionalRestrictionsJson: return directly
- If multiple certificates: use combineRestrictions
- Track source ("single_certificate" or "combined") and certificateCount

6. Handle case where certificates exist but none have functionalRestrictionsJson:
...
Run `npm test` - existing tests pass.
  ...
  <done>
storage.getCurrentRestrictions method exists and can query/combine restrictions from medical certificates.
  </done>
</task>
...
4. storage.getCurrentRestrictions method exists
