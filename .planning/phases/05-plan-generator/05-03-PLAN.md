...
```
  ...
  <verify>
TypeScript compiles for each component:
- `npx tsc --noEmit client/src/components/rtw/PlanTypeSelector.tsx`
- `npx tsc --noEmit client/src/components/rtw/ScheduleEditor.tsx`
- `npx tsc --noEmit client/src/components/rtw/DutySelector.tsx`
- `npx tsc --noEmit client/src/components/rtw/PlanPreview.tsx`
And full client build: `npm run build`
  </verify>
  <done>All four step components created with proper typing and UI patterns. Custom schedule editing available in ScheduleEditor per GEN-04.</done>
</task>
